...
class SampleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun onItemNameSelected(item : Person, position : Int) {
        val application = getApplication<SampleApplication>()
        Toast.makeText(application, "Person name ${item.name} tapped at position $position", Toast.LENGTH_SHORT).show()
        application.startActivity(Intent(application, ItemActivity::class.java).putExtra("id", item.id))
    }
    fun onItemAgeSelected(item : Person, position : Int) {
        val application = getApplication<SampleApplication>()
        Toast.makeText(application, "Person age ${item.age} tapped at position $position", Toast.LENGTH_SHORT).show()
        application.startActivity(Intent(application, ItemActivity::class.java).putExtra("id", item.id))
    }
}
